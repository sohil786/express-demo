...
function verifyToken(req, res, next) {
  ...
  if (token) {
    ...
    if (verifyResult && verifyResult.id) {
      ...
      return res.status(httpStatus.FORBIDDEN).send('Token failed verification');
    }
  } else {
    return res.status(httpStatus.FORBIDDEN).send('Token not provided as expected in authorization header');
  }
}
